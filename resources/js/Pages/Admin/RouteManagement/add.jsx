...
const AddRoute = ({ setShowAddModal }) => {
  ...
  const handleSubmit = (e) => {
    ...
    post(route('admin.route-management.store'), {
      data: formData,
      onSuccess: () => {
  ...
  return (
    <Dialog open onOpenChange={setShowAddModal}>
      <DialogContent className="w-[95vw] max-w-[1000px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-zinc-800">Add Route</DialogTitle>
          <DialogDescription className="text-zinc-600">
            Enter the details for the new route below.
          </DialogDescription>
        </DialogHeader>
            ...
            {stops.length === 0 ? (
              ...
              <div className="space-y-4">
                {stops.map((stop, index) => (
                  <div key={index} className="border rounded-lg p-4 bg-zinc-50">
